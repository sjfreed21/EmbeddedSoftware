/**
 * @file letimer.c
 * @author Sam Freed (and Keith Graham)
...
void letimer_pwm_open(LETIMER_TypeDef *letimer, APP_LETIMER_PWM_TypeDef *app_letimer_struct){
	...
	// XX X are values passed into the driver via app_letimer_struct
	...
	if(app_letimer_struct->comp0_irq_enable) {
		letimer->IEN |= app_letimer_struct->comp0_cb;
	}
 ...
 *
 * @details
 *	This function is what truly enables the LETIMER once the structure has been
 *	configured by the letimer_pwm_open() function.
 *
 * @note
 *	This function should only be called after the letimer_pwm_open() function has completed.
 *
 * @param[in] letimer
 *	LETIMER structure as defined by the documentation.
 *
 * @param[in] enable
 * 	Boolean true/false that either starts or stops the LETIMER.
 *
 ******************************************************************************/
...
/********************************************//**
 * @brief
 *	The IRQ Handler for LETIMER0.
 *
 * @details
 *	This function first reads and clears the interrupt flag register, then adds whichever
 *	interrupt had occurred into the scheduler.
 *
 * @note
 * 	scheduler_open() must have been called in order to enable the scheduler structure.
 *
 *******************************************/
void LETIMER0_IRQHandler(void) {
	uint32_t int_flag;

	// read source interrupts
	int_flag = LETIMER0->IF & LETIMER0->IEN;

	// clear IF register
	LETIMER0->IFC = int_flag;

	// interrupt handling if statements
