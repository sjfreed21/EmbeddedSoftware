...
/***************************************************************************//**
 * @brief
 *	Beginning to enable the HM-10 peripheral.
 *
 * @details
 * 	This function sets up the open struct and passes it to the LEUART open function.
 *
 * @note
 *  All uppercase globals are defined in ble.h.
 *
 * @param[in] tx_event
 *  The value of the TX callback event used in the scheduler.
 *
 * @param[in] rx_event
 *  The value of the RX callback event used in the scheduler.
 *
 ******************************************************************************/

void ble_open(uint32_t tx_event, uint32_t rx_event){
	LEUART_OPEN_STRUCT leuStruct;

	leuStruct.baudrate = HM10_BAUDRATE;
	leuStruct.databits = HM10_DATABITS;
	leuStruct.enable = HM10_ENABLE;
	leuStruct.parity = HM10_PARITY;
	leuStruct.rx_done_evt = rx_event;
	leuStruct.rx_en = true;
	leuStruct.rx_loc = LEUART0_RX_ROUTE;
	leuStruct.rx_pin_en = LEUART_ROUTEPEN_RXPEN;
	leuStruct.stopbits = HM10_STOPBITS;
	leuStruct.tx_done_evt = tx_event;
	leuStruct.tx_en = true;
	leuStruct.tx_loc = LEUART0_TX_ROUTE;
	leuStruct.tx_pin_en = LEUART_ROUTEPEN_TXPEN;

	leuart_open(HM10_LEUART0, &leuStruct);
}


/***************************************************************************//**
 * @brief
 *  Sends data to the LEUART.
 *
 * @details
 *  This function calls leuart_start, which sets up the LEUART state machine
 *  with the given string.
 *
 * @note
 *  HM10_LEUART0 is defined in ble.h.
 *
 * @param[in] string
...
bool ble_test(char *mod_name){
	...
	// ANSWER: Polling requires the system to continually check on the status of a peripheral and
	//         takes up more CPU processing power than waiting for an interrupt does.
	// How does interrupts benefit the system for low energy operation?
	// ANSWER: Interrupts mean that the system can focus on other processes or go to a lower sleep mode
	//		   to save energy instead of having to remain in a higher state or focused on polling to
	//         properly handle data from a peripheral.
	// How does interrupts benefit the system that has multiple tasks?
	// ANSWER: With interrupts, a system can execute other tasks while the interrupts are not asserted and
	//		   still receive the necessary data for processing. Using polling, on the other hand, forces
	//		   the system to focus on one specific process (that of communication with a peripheral) and blocks
...
//	leuart_cmd_write(HM10_LEUART0, (LEUART_CMD_CLEARRX | LEUART_CMD_CLEARTX));
	...
	// DSD HM10 module?
	// ANSWER: Bluetooth does not allow for "hotswapping" of device names, so all devices that
	//         are connected to a device being renamed must disconnect, let the device rename itself,
	// 		   and then reestablish the protocol. The test command also functions as the disconnect command,
	//		   which is why test_str ("AT") is used.
	...
	// ANSWER: If there is a device connected when the test/disconnect command is sent and the AT+NOTI
	//         parameter is set to notify, the response would instead be "OK+LOST".
