...
I2C_STATE_MACHINE i2c0_state;
I2C_STATE_MACHINE i2c1_state;
...
static void i2c_ack_sm(I2C_STATE_MACHINE *i2c){
	...
	case Write_CMD:
		if(i2c->bytes == 1) {
			...
			else if(i2c->bytes_tx == 1) {
				i2c->state = End_Sense;
				i2c->I2Cn->TXDATA |= *(i2c->data_addr) << (8 * i2c->bytes-1);
			}
		}
		break;
	case Wait_Read:
	...
	case End_Sense:
		...
		if(i2c->I2Cn == i2c1_state.I2Cn) {
			...
			if(i2c->bytes > 0) {
				...
				*i2c->data_addr |= i2c->I2Cn->RXDATA << 8;
				i2c->I2Cn->CMD = I2C_CMD_NACK;
				i2c->I2Cn->CMD = I2C_CMD_STOP;
				i2c->state = Stop;
			}
 ...
 *
 * @details
 *	Returns the private boolean of the state machine's busy status.
 *
 * @note
 *	Requires the state machine to be initialized.
 *
 * @param[in] i2c
 *  The I2C system currently being checked.
 ***************************************************/
bool i2c_check_busy(I2C_TypeDef *i2c){
	if(i2c == I2C0 && i2c == i2c0_state.I2Cn) return i2c0_state.busy;
	if(i2c == I2C1 && i2c == i2c1_state.I2Cn) return i2c1_state.busy;
	return true;
}
...
void I2C1_IRQHandler(void) {
	...
	if(int_flag & I2C_IF_ACK) {
		...
		i2c_ack_sm(&i2c1_state);
	}
	if(int_flag & I2C_IF_NACK) {
		EFM_ASSERT(!(I2C1->IF & I2C_IF_NACK));
		i2c_nack_sm(&i2c1_state);
	}
	if(int_flag & I2C_IF_MSTOP) {
		i2c_mstop_sm(&i2c1_state);
	}
	if(int_flag & I2C_IF_RXDATAV) {
		EFM_ASSERT(!(I2C1->IF & I2C_IF_MSTOP));
		i2c_rxdatav_sm(&i2c1_state);
	}
}
 ...
 *
 *	@param[in] bytes
 *	The number of bytes expected to be read from/written to the peripheral.
 *
 *	@param[in] cb
...
void i2c_start(I2C_TypeDef *i2c, uint32_t target_addr, uint32_t target_reg, bool w_r, uint32_t *data_addr, uint32_t bytes, uint32_t cb) {
	...
	if(i2c == I2C0) {
		i2c0_state.state = Start_CMD;
		i2c0_state.I2Cn = i2c;
		i2c0_state.target_addr = target_addr;
		i2c0_state.target_reg = target_reg;
		i2c0_state.w_r = w_r;
		i2c0_state.data_addr = data_addr;
		i2c0_state.bytes = bytes;
		i2c0_state.bytes_tx = 0;
		i2c0_state.I2Cn->CMD = I2C_CMD_START;
		i2c0_state.I2Cn->TXDATA = (i2c0_state.target_addr << 1) | I2C_WRITE;
		i2c0_state.busy = true;
		i2c0_state.cb = cb;
	} else if(i2c == I2C1) {
		i2c1_state.state = Start_CMD;
		i2c1_state.I2Cn = i2c;
		i2c1_state.target_addr = target_addr;
		i2c1_state.target_reg = target_reg;
		i2c1_state.w_r = w_r;
		i2c1_state.data_addr = data_addr;
		i2c1_state.bytes = bytes;
		i2c1_state.bytes_tx = 0;
		i2c1_state.I2Cn->CMD = I2C_CMD_START;
		i2c1_state.I2Cn->TXDATA = (i2c1_state.target_addr << 1) | I2C_WRITE;
		i2c1_state.busy = true;
		i2c1_state.cb = cb;
	}
}
